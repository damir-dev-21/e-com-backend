...
from .models import User,Item,Order,ItemEncoder
...
class ProductUpdateView(APIView):
    ...
    def put(self,request,id):
        ...
        if user.is_superuser:
            ...
            return Response({"Объект обновлен": f"Товар c id {id} обновлен"}, status=status.HTTP_426_UPGRADE_REQUIRED)
        else:
...
class ProductDeleteView(APIView):
    ...
    def delete(self,request,id):
        ...
        if user.is_superuser:
            product = Item.objects.get(id=id)
            product.delete()
            return Response({"Объект удален": f"Товар c id {id} удален"}, status=status.HTTP_204_NO_CONTENT)
        else:
            return Response({"Ошибка авторизации":"Вы не авторизованы как админ"})
